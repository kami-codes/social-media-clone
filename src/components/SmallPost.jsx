  ...
  const handleNavigate =()=>{
    ...
    return (
      <div className="small-post-container">
        <div className="small-post-profile-container">
        <div className="small-post-profile">
          <img
      ...
      <p className="small-post-time"> {postTime} </p>
        </div>
        <img className="small-post-photo" src={props.photo} alt="" width={200} />
        <div className="small-post-details">
  
     
        <div className="small-engagement-buttons">
        <button className="small-post-like-button" onClick={handleLike}>{isLiked ? <FavoriteOutlinedIcon style={{fontSize: "1.5rem"}} /> : <FavoriteBorderOutlinedIcon style={{fontSize: "1.5rem"}} /> }</button>
        <button disabled={commentsonPost.length === 0} onClick={()=>{setCommentCollapse(!commentCollapse)}} className="small-post-like-button"> <CommentIcon style={{fontSize: "1.5rem"}}  /> </button> 
        </div>
        <p className="small-post-likes" > {likes.length} likes </p>
        {captionCollapse ?  <p className="small-post-caption"> {props.caption.slice(0,50)} </p> : <p> {props.caption} </p> }
        {
          props.caption.length > 50 && 
        <p className="small-post-caption-control" onClick={()=>{setCaptionCollapse(!captionCollapse)}}> {captionCollapse ? "view more" : "view less"} </p>
        }
       ...
       <div className="small-post-comment">
        ...
        />
        <button className="small-comment-send-button" disabled={commentDesc.length <= 3} onClick={handleComment}> <SendIcon /> </button>
       </div>
